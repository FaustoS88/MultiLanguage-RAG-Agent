...
import asyncpg
import httpx # httpx is not explicitly used here, but pydantic-ai might use it internally
from openai import AsyncOpenAI
from pydantic import BaseModel, Field
from pydantic_ai import Agent, RunContext
from pydantic_ai.models.openai import OpenAIModel # Using OpenAIModel for OpenRouter compatibility
...
async def database_connect() -> AsyncGenerator[asyncpg.Pool, None]: # Keep None possibility for initial connection failure
    ...
    try:
        ...
        # We don't yield pool here, the context manager will exit.
        # The caller ('async with') will receive the exception.
        raise # Re-raise the connection error so the caller knows
    # REMOVED the generic except Exception block that was printing the weird message.
    # Exceptions from the 'yield pool' block will now propagate out naturally.
    finally:
...
# We use Pydantic-AI's OpenAIModel but configure its provider to point to OpenRouter.
try:
